...
namespace HMS.Controllers
{
    ...
    public class RptExpensesController : Controller
    {
        ...
        public RptExpensesController(ApplicationDbContext context, ICommon iCommon)
        {
            _context = context;
            _iCommon = iCommon;
        }
        ...
        public IActionResult GetDataTableData()
        {
            try
            {
                ...
                IQueryable<ExpensesGridViewModel> _GetGridItem = null;
                if (_StartDate != null && _EndDate != null && _StartDate != "" && _EndDate != "")
                {
                    _GetGridItem = _iCommon.GetExpensesGridItem().Where(x => x.CreatedDate >= Convert.ToDateTime(_StartDate) && x.CreatedDate <= Convert.ToDateTime(_EndDate).AddDays(1));
                }
                else
                {
                    _GetGridItem = _iCommon.GetExpensesGridItem();
                }
